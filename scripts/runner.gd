...
var _hp: int = CONSTANTS.RUNNER_MAX_HP
...
func _ready() -> void:
	...
	_hp = int(float(CONSTANTS.RUNNER_MAX_HP) * GameState.get_enemy_hp_multiplier())
	_health_bar = HEALTH_BAR_SCENE.instantiate() as Node2D
	_health_bar.position = Vector2(0.0, -36.0)
	_health_bar.call("setup", _hp, 22.0)
...
func _process(delta: float) -> void:
	...
	if dist <= CONSTANTS.RUNNER_ATTACK_RANGE:
		if not _attacking:
			_attacking = true
			_attack_timer = 0.0
			_play_anim("Attack")
		_attack_timer += delta
		if _attack_timer >= CONSTANTS.RUNNER_ATTACK_INTERVAL:
			_attack_timer = 0.0
			_target_node.call("take_damage", int(float(CONSTANTS.RUNNER_ATTACK_DAMAGE) * GameState.get_enemy_damage_multiplier()))
	else:
		if _attacking:
			_attacking = false
			_play_anim("Walk")
		global_position += to_target.normalized() * CONSTANTS.RUNNER_SPEED * delta
...
func _die() -> void:
	GameState.add_coins(CONSTANTS.RUNNER_COIN_REWARD)
...
func _show_coin_popup() -> void:
	...
	label.text = "+%d" % CONSTANTS.RUNNER_COIN_REWARD
	label.add_theme_font_size_override("font_size", 13)
	label.add_theme_color_override("font_color", Color(1.0, 0.85, 0.2))
	label.position = Vector2(-16.0, -38.0)
	add_child(label)
	var tween: Tween = create_tween()
	tween.tween_property(label, "position:y", -62.0, 0.7)
