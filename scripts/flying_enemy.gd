...
const PROJECTILE_SCENE: PackedScene = preload("res://prefabs/enemy_projectile.tscn")

@onready var _sprite: AnimatedSprite2D = $AnimatedSprite2D

var _hp: int = CONSTANTS.FLYING_ENEMY_MAX_HP
...
func _process(delta: float) -> void:
	...
	# Gentle hover bob to distinguish from ground enemies
	_hover_time += delta
	if _sprite != null:
		_sprite.position.y = sin(_hover_time * 3.5) * 5.0
	...
	if dist <= CONSTANTS.FLYING_ENEMY_ATTACK_RANGE:
		_attack_timer += delta
		if _attack_timer >= CONSTANTS.FLYING_ENEMY_ATTACK_INTERVAL:
			_attack_timer = 0.0
			_fire_projectile()
	else:
		global_position += to_target.normalized() * CONSTANTS.FLYING_ENEMY_SPEED * delta
		if _sprite != null:
...
func _show_coin_popup() -> void:
	...
	label.text = "+%d" % CONSTANTS.FLYING_ENEMY_COIN_REWARD
