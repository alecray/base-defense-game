...
var _shield_mode: bool = false

const BAR_HEIGHT: float = 5.0

func setup(max_hp: int, bar_width: float = 48.0, shield_mode: bool = false) -> void:
	_max_hp = max_hp
	_current_hp = max_hp
...
func _draw() -> void:
	...
	if _shield_mode:
		draw_rect(Rect2(x, y, _bar_width, BAR_HEIGHT), Color(0.0, 0.05, 0.2, 0.9))
		var intensity: float = lerpf(0.3, 1.0, ratio)
		draw_rect(Rect2(x, y, _bar_width * ratio, BAR_HEIGHT), Color(0.1, intensity * 0.55, intensity))
	else:
