	...
	return [
		{
			"id": "turret_damage",
			"name": "Turret Damage",
			"cost": CONSTANTS.LIBRARY_RESEARCH_TURRET_DAMAGE_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_TURRET_DAMAGE_DURATION,
			"one_time": false,
			"desc": "+%d tower damage per level. No cap." % CONSTANTS.LIBRARY_TURRET_DAMAGE_UPGRADE_AMOUNT,
		},
		{
			"id": "armory_blueprint",
			"name": "Armory Blueprint",
			"cost": CONSTANTS.LIBRARY_RESEARCH_ARMORY_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_ARMORY_DURATION,
			"one_time": true,
		...
		},
		{
			...
			"cost": CONSTANTS.LIBRARY_RESEARCH_SOLDIER_DAMAGE_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_SOLDIER_DAMAGE_DURATION,
			"one_time": false,
			"desc": "+%d soldier attack damage per level. No cap." % CONSTANTS.LIBRARY_SOLDIER_DAMAGE_UPGRADE_AMOUNT,
		},
		{
			"id": "soldier_hp",
			"name": "Soldier Endurance",
			"cost": CONSTANTS.LIBRARY_RESEARCH_SOLDIER_HP_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_SOLDIER_HP_DURATION,
			"one_time": false,
			"desc": "+%d soldier max HP per level (applies to newly spawned soldiers)." % CONSTANTS.LIBRARY_SOLDIER_HP_UPGRADE_AMOUNT,
		},
		{
			"id": "worker_speed",
			"name": "Efficient Workers",
			"cost": CONSTANTS.LIBRARY_RESEARCH_WORKER_SPEED_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_WORKER_SPEED_DURATION,
			"one_time": false,
			"desc": "+%d worker travel speed per level. No cap." % int(CONSTANTS.LIBRARY_WORKER_SPEED_UPGRADE_AMOUNT),
		},
		{
			"id": "farm_yield",
			"name": "Agricultural Methods",
			"cost": CONSTANTS.LIBRARY_RESEARCH_FARM_YIELD_COST,
			"duration": CONSTANTS.LIBRARY_RESEARCH_FARM_YIELD_DURATION,
			"one_time": false,
			"desc": "+%d food per farm worker per harvest tick per level." % CONSTANTS.LIBRARY_FARM_YIELD_UPGRADE_AMOUNT,
		},
	]
...
func open_for_library(_gp: Vector2i, _building_node: Node) -> void:
	_refresh()
	visible = true

func _refresh() -> void:
	var busy: bool = not GameState.library_research_id.is_empty()
	var active_id: String = GameState.library_research_id
	...
	for def: Variant in _get_research_defs():
		...
		if done:
			...
			if is_active:
				var progress: float = GameState.library_research_progress / duration
